...
> **苹果冷调灰白底 + 实心白内容表面 + 发丝分隔线；液态玻璃是控制层表面的主材质（导航、工具栏、侧边栏、弹层、popover、tab bar、开关）。克制即奢侈。**
...
- **双轨材质：玻璃在内容上是调味，在控制层上是主材质。** 内容表面（文章、列表、表单输入框）保持实心白 + 柔和阴影。控制表面（导航、工具栏、侧边栏、弹层、popover、tab bar、开关、浮动按钮）使用液态玻璃作为主材质——动态的、状态感知的、内容自适应的。
- **层级靠字重 + 字号 + 灰阶，不靠颜色。** 正文是黑白灰；颜色**只做 accent**（一个蓝）。
- **克制即奢侈（内容表面上）。** 一千个「不」换一个「是」。先用留白和层级解决，再谈加边框/填充/图标。
- **质感在细节。** 大标题负字距、`tabular-nums`、发丝线、轻悬停抬升、同心圆角、状态感知玻璃变形（rest→active→scrolled）、5 级降级阶梯。
...
整套**与框架无关**：文件假设纯 HTML/CSS。在 React/Vue/Svelte 里，把 CSS 翻成你的样式方案，但**保留精确的 token 值**、面板而非卡片的模式、以及双轨玻璃规则（内容用实心白，控制层用液态玻璃）。
...
| `design-system.md` | 完整规格：色彩、字号阶、间距/圆角/阴影档位、液态玻璃配方族（两种基础材质、状态系统、暗色模式、降级阶梯、同心圆角、z-index）、响应式。 |
| `tokens.css` | `:root` 自定义属性——放进任何项目，用 `var(--…)` 引用。包含 `--glass-*` 材质/状态 token 和 `--z-*` z-index 阶梯。 |
| `components.md` | 可直接粘贴的 HTML+CSS：玻璃导航、统一面板列表、卡片网格、分段控件、标签、按钮、彩色 CTA、开关、在线点，加上玻璃控制层组件集（工具栏、tab bar、侧边栏、开关、滑块、popover、tooltip、FAB、复选框/单选、上下文菜单）。 |
...
- 同类项 → **一张面板 + 发丝线**，不是碎片卡片。
- **双轨表面**：内容（文章、列表、表单输入框）是实心白 + 阴影；控制表面（导航、工具栏、侧边栏、弹层、popover、开关）使用液态玻璃 + `var(--glass-*)` token + `var(--z-*)` z-index。
